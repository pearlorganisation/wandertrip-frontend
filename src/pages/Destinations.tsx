...
import { fetchDestinations } from "@/lib/redux/Action/destinationAction";
...
const mockDestinations = [
  ...
  },
  {
    ...
    location: "Morocco",
    image_url:
      "https://images.unsplash.com/photo-1597211684565-dca64d72bdfc?auto=format&fit=crop&w=800&q=80",
    rating: 4.4,
    is_hidden: true,
  },
  {
...
const Destinations = () => {
  ...
  const { destinations: savedDestinations, toggleSaveDestination } =
    useSavedDestinations();

  const [activeFilter, setActiveFilter] = useState("All");
  const [searchQuery, setSearchQuery] = useState("");
  const [showHiddenOnly, setShowHiddenOnly] = useState(false);
  const [destinationsnew, setDestinations] = useState<any[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  const { destinations, pagination } = useAppSelector(
  ...
  useEffect(() => {
    function getDestinations() {
      try {
        setIsLoading(true);
        // Simulate API delay
        // dispatch(fetchDestinations({ page, limit }));
  ...
  const filteredDestinations = destinations.filter((destinations) => {
    // Apply search filter
    const matchesSearch =
      destinations.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      destinations.country.toLowerCase().includes(searchQuery.toLowerCase());

    // Apply hidden gems filter
    const matchesHidden = showHiddenOnly ? destinations : true;
  ...
  const handlePageChange = (page) => {
    dispatch(fetchDestinations({ page, limit: 10 }));
  };
          ...
          <div className="container px-4 sm:px-6">
            <div className="max-w-3xl mx-auto text-center mb-12">
              ...
              <p className="text-muted-foreground text-lg">
                Explore unique places around the world tailored to your
                preferences
              </p>
            </div>
            ...
            {isLoading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[...Array(8)]?.map((_, index) => (
                  <div
                    key={index}
                    className="rounded-2xl overflow-hidden bg-secondary/30 animate-pulse h-96"
                  />
                ))}
              </div>
            ) : (
                ...
                {filteredDestinations?.map((destination, index) => (
                  ...
                  >
                    <DestinationCard
                      ...
                      isFavorite={getIsFavorite(destination._id)}
